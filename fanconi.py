...
for cond in CONDITIONS:
    ...
    if PYSB:
        t_end = 50
        n_runs = 100
        t_thresh = run_FA_Boolean_pysb(model_pysb, t_end, n_runs, param_values=param_values, verbose=False,
                                       outfile='%s_pysb.pdf' % prefix, **stoch_kwargs)
        mutations = [str(m) for mut in cond['mutations'] for m in mut]
        initials = [str(m) for mut in cond['initials'] for m in mut]
